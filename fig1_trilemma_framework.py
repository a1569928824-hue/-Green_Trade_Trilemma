#!/usr/bin/env python3
"""
Figure 1 — Green Trade Trilemma conceptual framework (v12)
============================================================
Round 7 — user-verified occlusion + Nature-editor refinements:

  1. Panel c: removed oversized annotation bbox; restored staggered
     right-edge % labels with generous spacing.
  2. Panel b: all label offsets increased to 3–8 data units to clear
     markers and uncertainty bands. % labels placed consistently below
     all lines to avoid convergence.
  3. Panel b: "GSI declining" annotation raised +12→+25.
  4. Panel c: legend ncol=2→1; ylim [0.47,1.42]→[0.44,1.44].
  5. Panel b: convention legend raised to gdi_ylo+15.
  6. Panel c: GEI label below line to avoid policy-annotation overlap;
     compact "2015 → 2024 change" reference note in lower-left.
  7. Panel b: Dev (GDI) label offset reduced +5.5→+3 (fixes clipping
     at ylim boundary).

Export: SVG (editable) + PDF + TIFF 600 dpi
"""
...
def draw_panel_a(ax):
    ...
    ax.text(0, -0.08, "Trilemma\nzone", fontsize=6, color=C_NEUTRAL,
...
# ═══════════════════════════════════════════════
# PANEL B — GDI × GSI by income group (EQUAL-SPAN dual axes)
# ═══════════════════════════════════════════════
...
def draw_panel_b(ax):
    ...
    ax.set_ylabel("Import-source diversity (GDI)\nNormalised index (2015 = 100)",
                  fontsize=7, color=C_DIVER, labelpad=3)
    ...
    ax2.set_ylabel("Deployment speed (GSI)\nNormalised index (2015 = 100)",
                    fontsize=7, color=C_SPEED, labelpad=3)
    ...
    yr_idx_2016 = list(years).index(2016)
    ax.text(2017.5, dev_gdi_pct[yr_idx_2016] - 5.0, "Dev (GDI)", fontsize=6,
            color=C_DIVER, fontweight="bold", ha="left", va="top")
    ax.text(2015.8, hi_gdi_pct[yr_idx_2016] - 5.0, "HI (GDI)", fontsize=6,
            color=C_DIVER, fontweight="bold", ha="left", va="top")

    yr_idx_2019 = list(years).index(2019)
    ax2.text(2019.1, dev_gsi_pct[yr_idx_2019] + 8, "Dev (GSI)", fontsize=6,
             color=C_SPEED, fontweight="bold", ha="left", va="bottom")
    ax2.text(2019.1, hi_gsi_pct[yr_idx_2019] - 8, "HI (GSI)", fontsize=6,
             color=C_SPEED, fontweight="bold", ha="left", va="top")

    # ---- % change labels at right edge, 6pt ----
    # All labels placed below their respective lines (consistent direction,
    # avoids convergence where Dev and HI lines approach each other).
    for arr, pct, col, dy in [
        (ax,  dev_gdi_pct, C_DIVER,  -5.5),
        (ax,  hi_gdi_pct,  C_DIVER,  -5.5),
        (ax2, dev_gsi_pct, C_SPEED,  -8),
        (ax2, hi_gsi_pct,  C_SPEED,  -8),
    ]:
        arr.text(2024.3, pct[-1] + dy,
...
def draw_panel_c(ax):
    ...
    ax.set_xlabel("Year", fontsize=7, labelpad=2)
    ax.set_ylabel("Normalised index (2015 ≡ 1)", fontsize=7, labelpad=2)
    ax.axhline(1.0, color=C_NEUTRAL, lw=0.6, ls=":", alpha=0.45, zorder=1)
    ...
    annotations = [
        (2020, gdi_norm[yr_idx[2020]], -0.13,
         "US Section 201\nextension +\nIndia PLI launch", "below"),
        (2022, gdi_norm[yr_idx[2022]], -0.13,
         "US IRA + India BCD;\nEU CBAM negotiations\nbegin", "below"),
        (2024, gei_norm[yr_idx[2024]], +0.07,
         "EU NZIA + Canada\n100% EV tariff\n+ UK CBAM", "above"),
    ]
    for yr_ev, y_data, dy, label, direction in annotations:
        if direction == "above":
            y_txt = y_data + abs(dy)
            va = "bottom"
        else:
    ...
    ax.text(0.02, 0.04, "2015 → 2024 change",
...
def main():
    print("=" * 54)
    print("Figure 1 — Green Trade Trilemma v12 (Round 8)")
    print("=" * 54)
